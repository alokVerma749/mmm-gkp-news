...
export const CarouselCard = ({ article }: { article: Article }) => {
  return (
    <Link href={`/article/${article._id}`} className="w-[100dvw] relative h-[30dvh] min-w-full snap-start">
      {article.image && (
        <Image alt="not found" src={article.image} height={100} width={100} className="w-full h-full object-cover object-center" />
      )}
